...
NTL::ZZ Heratio::DotProduct(NTL::Vec<NTL::ZZ> v1, NTL::Vec<NTL::ZZ> v2, NTL::ZZ prime)
{
...
{
  return NTL::power_long(this->lambda, 2);
}
...
NTL::ZZ Heratio::GenerateQ()
{
  return NTL::GenPrime_ZZ(this->mu, 80);
}
...
NTL::ZZ Heratio::GenerateQStar()
{
  ...
  long q_star_bits = long((this->beta * this->mu - ceil(log2(t)) + 2 * this->d) / d) - 1;
  NTL::ZZ q_star = NTL::GenPrime_ZZ(q_star_bits, 80);
...
NTL::ZZ Heratio::ComputeQToBeta()
{
  return NTL::power(this->q0, this->beta);
}

NTL::ZZ Heratio::ComputeX0()
{
